...
typedef class Program_class *Program;	////Program是一个指向class Program_class的一个指针（同typedef struct xx* xx）

class Program_class : public tree_node {
public:
   tree_node *copy()		 { return copy_Program(); }	////把子类指针返回给父类指针。在tree_node类（纯虚函数，是抽象类（不能被实例化））中，copy()是虚函数，动态绑定所属类（执行时绑定）。而java默认动态绑定
   virtual Program copy_Program() = 0;	////纯虚函数，是抽象类（不能被实例化），必须外部继承
...
typedef class Class__class *Class_;	////后面可以看到Classes是list_node<Class_>的指针即typedef class list_node<Class__class*>* Classes
...
typedef Classes_class *Classes;	////Classes_class代表Classes_类，Classes就代表他的指针也就是说Classes是一个class list_node<Class__class*>*类型
...
class program_class : public Program_class {	////重写了抽象类
protected:
   Classes classes;
public:
...
class class__class : public Class__class {	////本质上这些都是继承自tree_node
protected:
   Symbol name;
   Symbol parent;
   Features features;
